...
def operate_on_DB(dbname, username, site, status, group):
    ...
    with closing(sqlite3.connect(dbname)) as DBconnection:
        with closing(DBconnection.cursor()) as cursor:
            ...
            cursor.execute(DB.insert_user, (username, site, status, date))
...
def update_userstatus(dbname, status, username, site):
    ...
    if not isinstance(username, str):
        for user in username:
            with closing(sqlite3.connect(dbname)) as DBconnection:
                with closing(DBconnection.cursor()) as cursor:
                    cursor.execute(DB.update_userstatus, (status, user, site))
                    DBconnection.commit()
        ...
        with closing(sqlite3.connect(dbname)) as DBconnection:
            with closing(DBconnection.cursor()) as cursor:
                cursor.execute(DB.update_userstatus, (status, username, site))
                DBconnection.commit()
...
def display_revoked_users(dbname):
    ...
    formatted_result = [
        f"{cnt:<5}{username:<30}{site:<20}{status:<20}{date:<15}"
    ...
    print(
        "\n".join(
            [f"{Count:<5}{username:<30}{site:<20}{status:<20}{date:<15}"]
            + formatted_result
        )
    )
...
def display_user_groups(dbname):
    ...
    with closing(sqlite3.connect(dbname)) as DBconnection:
        with closing(DBconnection.cursor()) as cursor:
            username_rows = cursor.execute(DB.username_query, (user,)).fetchall()
            ...
            for usrIDs in username_rows:
                for usrID in usrIDs:
                    group_query = cursor.execute(DB.group_query, (usrID,)).fetchall()
